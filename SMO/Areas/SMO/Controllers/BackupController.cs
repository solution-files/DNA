...
namespace SMO.Controllers {
    ...
    public class BackupController : Controller {
        ...
        public async Task<IActionResult> Index() {
			string message;
			try {
				var result = await Context.Databases.FromSqlRaw("SELECT Database_ID, [Name], User_Access_Desc, Create_Date FROM sys.databases WHERE Database_ID > 4").ToListAsync();
        ...
        public IActionResult Run([FromBody]IList<Databases> instance) {
			try {
				ServerConnection cn = new ServerConnection() {
					ServerInstance = Configuration["SMO:ServerInstance"],
					LoginSecure = Convert.ToBoolean(Configuration["SMO:LoginSecure"]),
					Login = Configuration["SMO:Login"],
					Password = Configuration["SMO:Password"]
                };
				cn.Connect();
